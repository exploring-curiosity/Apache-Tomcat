    ...
    public void doGet(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {
        ...
        try
        {
            ...
            try
            {
                ...
                try
                {
                    String del = " DELETE FROM Patient_Details WHERE ID="+(String)request.getParameter("id");
                    ps=con.prepareStatement(del);
                    int af_rows = ps.executeUpdate();
                    if(af_rows==1){
                        pw.println("<script>");
                        pw.println("alert(\"deleted Successfully \");");
                        pw.println("location='http://localhost:8080/LabEX4/delete.html';");
                        pw.println("</script>");
                    }
                    else
                    {
